...
function seedDocuments(db: Database.Database): void {
  ...
  ins.run(107,'Warm Lead Follow-Up Script','Sales','# Warm Lead Follow-Up Script\n\n## Purpose\nScript and process for following up with Contacted and Interested leads who have not responded in 3+ days.\n\n## When to Use\nEvery Wednesday at 10:00 AM. Pull all leads with status Contacted or Interested and follow_up_date on or before today.\n\n## Phone Script\n"Hi, this is Chris from OnTrack Hauling Solutions. I reached out a few days ago about our dispatch service. I wanted to follow up and see if you had any questions or if the timing is better now. We work with owner-operators on a 7% dispatch rate -- no contracts, just results. Do you have 5 minutes to chat?"\n\n## Text/DM Script\n"Hey [Name], just following up from my message earlier this week. Still interested in talking about dispatch? Happy to answer any questions. No pressure at all."\n\n## Email Script\nSubject: Following up -- OnTrack Dispatch Services\n\n"Hi [Name],\n\nI wanted to follow up on my earlier message about OnTrack Hauling Solutions dispatch services.\n\nWe help owner-operators find quality loads, negotiate rates, and handle broker paperwork -- all for 7% of gross revenue with no long-term contracts.\n\nIf you have a few minutes this week, I would love to connect. What does your schedule look like?\n\nBest,\nChris Hooks\nOnTrack Hauling Solutions\ndispatch@ontrackhaulingsolutions.com"\n\n## Follow-Up Rules\n- Attempt 1: Day 3 after initial contact (phone or DM)\n- Attempt 2: Day 7 (email or text)\n- Attempt 3: Day 14 (final call)\n- After 3 attempts with no response: mark lead as Rejected with note "No response after 3 attempts"\n\n## Tracking\nUpdate lead notes after every contact attempt with date, method, and outcome.',null,null,null)
  ins.run(108,'FMCSA Lead Review Checklist','Sales','# FMCSA Lead Review Checklist\n\n## Purpose\nWeekly Monday process for reviewing FMCSA-imported leads and prioritizing outreach.\n\n## Step 1 -- Run the Import\n1. Go to Leads page.\n2. Click "FMCSA Import" button.\n3. Wait for the import to complete. Note how many leads were added.\n\n## Step 2 -- Score New Leads\nFor each new lead, evaluate:\n- Fleet size: 2+ trucks = High priority. Solo = Medium. Unknown = Low.\n- Trailer type match: Flatbed, Reefer, Dry Van -- all acceptable. Specials (Tanker, HazMat) = lower priority.\n- Authority date: Less than 1 year old = hot lead, new to the industry, may not have dispatcher yet.\n- Location: Drivers in TX, TN, GA, IL, OH, MO = strong lane match for our network.\n\n## Step 3 -- Assign Follow-Up Dates\n- High priority: follow-up within 2 business days.\n- Medium priority: follow-up within 5 business days.\n- Low priority: follow-up within 10 business days.\n\n## Step 4 -- Update Statuses\n- Any lead you have already spoken to: move from New to Contacted.\n- Any lead that expressed interest: move to Interested.\n- Any duplicate you recognize: mark Rejected with note "Duplicate".\n\n## Step 5 -- Review Existing Pipeline\n- Check all Contacted leads for stale follow-up dates.\n- Check all Interested leads -- any ready to close this week?\n- Review any Rejected leads from last week for accuracy.\n\n## Notes\nTarget: review and score all new leads within 2 hours of import on Monday morning.',null,null,null)
...
export function reseedDocuments(db: Database.Database): void {
  ...
  // ── 131 ── Load Booking Checklist ─────────────────────────────────────────
  ups.run(131, 'Load Booking Checklist', 'Dispatch', [
    '# Load Booking Checklist',
    '',
    'Use this as a quick reference before and during every load booking.',
    'For the full step-by-step walkthrough, see [[Load Booking SOP]].',
    '',
    '## Before You Call',
    '- Driver is available and confirmed their location',
    '- Equipment type matches the load requirement',
    '- Driver insurance is current (check driver profile)',
    '- MC authority is active (verify on FMCSA SAFER)',
    '- Deadhead is under 100 miles (empty miles to pickup)',
    '- Rate per mile meets minimum ($2.00 dry van / $2.50 reefer / $2.25 flatbed)',
    '- Broker is not flagged as Avoid in the Brokers page',
    '',
    '## On the Call',
    '- Confirm load is still available',
    '- Get: all-in rate, pickup window, delivery window, commodity, weight',
    '- Counter 10-15% above posted rate before accepting',
    '- Say "I will call back in 5 minutes" if you need time to confirm with the driver',
    '',
    '## After the Driver Says Yes',
    '- Ask broker to email rate confirmation to dispatch@ontrackhaulingsolutions.com',
    '- Review rate con before forwarding to driver',
    '- Enter load in OnTrack: Loads > New Load',
    '- Update load status to Booked',
    '- Send driver the pickup address, time, shipper contact, commodity, and weight',
    '- Notify broker: driver name, MC#, truck/trailer number, driver cell, ETA to pickup',
    '',
    '## During the Load',
    '- Update status to Picked Up when driver loads',
    '- Update status to In Transit when driver is rolling',
    '- Log a check call at least once on hauls over 500 miles',
    '- Update status to Delivered when driver confirms delivery',
    '',
    '## Never Skip',
    '- Never book without driver confirmation',
    '- Never dispatch without a signed rate con',
    '- Never dispatch with expired insurance or suspended authority',
    'See [[Breakdown and Emergency Procedures]] if anything goes wrong mid-load.',
  ].join('\n'), null, null, null)

  // ── 132 ── Check Call Script ───────────────────────────────────────────────
  ups.run(132, 'Check Call Script', 'Dispatch', [
    '# Check Call Script',
    '',
    'A check call is a brief status check with your driver during an active load.',
    'The goal: confirm location, verify ETA, catch problems early.',
    '',
    '## When to Call',
    '- Once per day on loads under 500 miles',
    '- Twice per day on loads over 500 miles',
    '- Any time a broker requests a status update',
    '- Immediately if a driver misses a pickup or delivery window',
    '',
    '## Standard Check Call',
    '"Hey [Driver], just doing a quick check-in. Where are you right now?"',
    '',
    'After they answer:',
    '"Still on track for [City] by [Time]?"',
    '',
    'If on track: "Perfect. Call me if anything changes. I will check back in later."',
    '',
    '## If They Are Running Late',
    '"How far out do you think you are? I need to notify the broker."',
    '',
    'Then call or email the broker:',
    '"Hi, this is [Name] from OnTrack Hauling Solutions. Just an update on load [Ref#] -- the driver is running approximately [X] hours behind. Updated ETA is [Time]. Will keep you posted."',
    '',
    'Log the delay in the load notes in OnTrack.',
    '',
    '## If There Is a Problem (breakdown, accident, weather)',
    'See [[Breakdown and Emergency Procedures]] for the full response process.',
    '',
    '## After the Call',
    '- Log the check call in the load timeline: Loads > [Load] > Check Calls > Log Call',
    '- Note the location, ETA, and any issues',
    '- If broker is expecting delivery within 2 hours, proactively send them an update without waiting for them to ask',
    '',
    '## What to Log',
    'Every check call entry should include: driver location, ETA, any delays or issues, and broker notified (yes/no).',
  ].join('\n'), null, null, null)

  // ── 133 ── Invoice and Collections Guide ──────────────────────────────────
  ups.run(133, 'Invoice and Collections Guide', 'Finance', [
    '# Invoice and Collections Guide',
    '',
    '## When to Invoice',
    'Invoice as soon as the load is delivered and the POD (Proof of Delivery) is confirmed.',
    'Do not wait. Same-day invoicing keeps cash flow tight and reduces aging.',
    '',
    '## How to Create an Invoice',
    '1. Go to Invoices > New Invoice',
    '2. Select the load from the dropdown',
    '3. Confirm the gross rate and your dispatch fee (7% of gross)',
    '4. Set status to Draft, review, then change to Sent when you email it',
    '5. Note the broker payment terms in the invoice notes (Quick Pay / Net 30 / Net 45)',
    '',
    '## What to Include When Sending',
    'Email to the broker AP department (not the load rep unless the rep handles billing):',
    '- Your invoice as a PDF',
    '- The signed rate confirmation',
    '- The POD (signed bill of lading)',
    '- Your W-9 if it is your first invoice to that broker',
    '',
    'Subject line: "Invoice [Inv#] -- Load [Ref#] -- [Origin] to [Destination] -- [Carrier Name]"',
    '',
    '## Payment Terms',
    '- Quick Pay: 1-3 business days (broker charges 2-5%). Use only when cash flow is tight.',
    '- Standard Net 30: payment due 30 days from invoice date',
    '- Net 45 / Net 60: longer terms -- flag these brokers in the Brokers page',
    '',
    '## Following Up on Unpaid Invoices',
    '',
    '### 14-16 days past invoice (first follow-up)',
    '"Hi, this is [Name] from OnTrack Hauling Solutions. I am following up on invoice [Inv#] for load [Ref#] delivered on [Date]. Can you confirm the payment status?"',
    '',
    '### 25-28 days past invoice (second follow-up)',
    'Email AP with the original invoice attached again.',
    'Subject: "Second Notice -- Invoice [Inv#] -- Due [Date]"',
    'Body: "Please find the attached invoice for load [Ref#]. Payment was due [Date]. Please confirm payment status or advise if there is a hold."',
    '',
    '### 35+ days past invoice (escalate)',
    '- Call the broker load rep and ask them to escalate to AP',
    '- Update the broker record in OnTrack with a note about the payment issue',
    '- Consider flagging the broker as Slow Pay if this is a pattern',
    '',
    '## Using the Invoices Page',
    '- Draft: invoice created but not sent',
    '- Sent: invoice emailed to broker',
    '- Overdue: invoice is past due (visible in Reports > Accounts Receivable Aging)',
    '- Paid: mark as paid when payment hits your account',
    '',
    'The Reports page shows an Accounts Receivable Aging table grouped by how many days each invoice is outstanding.',
    'Review it every Wednesday as part of the weekly admin routine.',
    '',
    '## Related Documents',
    'See [[Invoice Submission Process]] for the full broker-specific invoicing steps.',
    'See [[Driver Settlements]] for how to calculate and pay driver earnings.',
  ].join('\n'), null, null, null)

  // ── 134 ── Broker Load Lifecycle ──────────────────────────────────────────────
  ups.run(134, 'Broker Load Lifecycle', 'Brokers', [
    '# Broker Load Lifecycle',
    '',
    '## Overview',
    'When OnTrack operates in broker mode you are arranging freight on behalf of a shipper and paying a carrier to haul it. This document covers the full lifecycle of a broker-mode load from intake through final payment.',
    '',
    '## Stage 1 -- Load Intake',
    'A shipper contacts OnTrack (or you source the freight directly) with a load that needs to be moved.',
    '',
    'Collect all required information:',
    '- Pickup address (full street address, city, state, zip)',
    '- Delivery address (full street address, city, state, zip)',
    '- Pickup date and appointment window',
    '- Delivery date and appointment window',
    '- Commodity description and total weight',
    '- Equipment type required (Dry Van, Flatbed, Reefer, etc.)',
    '- Total miles (use Google Maps or PC Miler to verify)',
    '- Rate you are being paid by the shipper (or your target rate if self-sourced)',
    '',
    'Create the load in OnTrack with status Posted.',
    '',
    '## Stage 2 -- Post to DAT',
    'Post the load to DAT to attract carrier offers. See [[DAT Load Posting Guide (Broker Mode)]] for step-by-step instructions.',
    '',
    'Set your target carrier pay rate:',
    '- Your margin: typically 10-20% of the shipper rate',
    '- Carrier pay = Shipper rate minus your margin',
    '- Example: shipper pays $2,500, carrier pay target is $2,100 (margin $400)',
    '',
    '## Stage 3 -- Receive and Evaluate Offers',
    'Carriers and dispatchers will call in response to the DAT posting. See [[Inbound Carrier Call Script]] for handling these calls.',
    '',
    'Evaluate each offer against:',
    '- Carrier vetting: authority age, insurance, safety record (see Carrier Vetting section below)',
    '- Equipment match: correct trailer type and length',
    '- Location: reasonable deadhead to pickup -- under 150 miles preferred',
    '- Rate: within your target carrier pay range',
    '- Communication quality: do they have the driver info ready?',
    '',
    '## Carrier Vetting Checklist',
    'Before booking any carrier confirm ALL of the following:',
    '',
    '### 1. FMCSA Authority Verification',
    'Go to safer.fmcsa.dot.gov and search by MC number:',
    '- Status: ACTIVE (not inactive, suspended, or revoked)',
    '- Operation: Authorized For-Hire',
    '- Authority age: minimum 30 days. Under 6 months requires extra scrutiny.',
    '- No OOS (Out of Service) orders',
    '',
    '### 2. Insurance Verification',
    '- Carrier must have active commercial liability insurance -- minimum $1,000,000',
    '- Cargo coverage -- minimum $100,000',
    '- OnTrack (or the shipper) must be listed as Certificate Holder on the COI',
    '- Request COI from carrier before load moves. No COI = no load.',
    '',
    '### 3. Safety Score Check',
    'Check at ai.fmcsa.dot.gov/SMS:',
    '- Unsafe Driving: avoid carriers above the 75th percentile',
    '- HOS Compliance: flag carriers with elevated scores',
    '- Any ALERT or intervention warning = do not use',
    '',
    '### 4. DAT Carrier Verification',
    'On DAT, check the carrier profile:',
    '- Cargo claims history',
    '- Rejection rate',
    '- Payment complaints from other brokers',
    '',
    '### 5. Carrier411 (Free Check)',
    'Search carrier411.com for the carrier MC number.',
    '- Any fraud or double-brokering flags = hard stop',
    '- Multiple load abandonment reports = do not use',
    '',
    '## Stage 4 -- Select Carrier and Book',
    'Once a carrier passes vetting:',
    '1. Confirm driver name, truck number, trailer number, and driver cell phone',
    '2. Confirm MC number matches FMCSA record',
    '3. Issue a rate confirmation to the carrier',
    '4. Receive signed rate con back before the truck moves',
    '5. Update load status in OnTrack to Booked',
    '6. Send full pickup instructions to carrier: shipper address, contact, appointment time, BOL instructions',
    '',
    '## Stage 5 -- Load in Transit',
    '- Carrier picks up: update load status to Picked Up',
    '- Request BOL number and pickup confirmation',
    '- Check call at least once on hauls over 400 miles',
    '- Update load status to In Transit when rolling',
    '- Notify shipper of pickup confirmation and estimated delivery',
    '',
    '## Stage 6 -- Delivery',
    '- Carrier delivers: update load status to Delivered',
    '- Request signed POD from carrier within 24 hours',
    '- Confirm delivery with shipper/receiver',
    '- Invoice the shipper for your rate',
    '- Pay the carrier per your agreed rate con terms',
    '',
    '## Stage 7 -- Payment',
    '- Collect payment from shipper (typically Net 30)',
    '- Pay carrier per rate con payment terms',
    '- Update invoice status to Paid when collected',
    '- Add or update the carrier record in OnTrack Brokers section with payment notes',
    '',
    '## Common Problems and How to Handle Them',
    '',
    '**Carrier no-shows:**',
    'If a carrier does not pick up and stops responding: mark the load back to Posted and re-post on DAT immediately. Flag the carrier as Avoid in OnTrack. Note the incident with date and MC number.',
    '',
    '**Rate renegotiation after booking:**',
    'If a carrier tries to renegotiate the rate after signing the rate con: hold firm. The signed rate con is a binding agreement. If they refuse to move the load at the agreed rate, find another carrier and flag this one.',
    '',
    '**Load cancellation by shipper:**',
    'If the shipper cancels after a carrier is dispatched and en route: the carrier may be entitled to TONU (Truck Order Not Used) pay. Review your shipper agreement for cancellation terms.',
  ].join('\n'), null, null, null)

  // ── 135 ── DAT Load Posting Guide (Broker Mode) ────────────────────────────
  ups.run(135, 'DAT Load Posting Guide (Broker Mode)', 'Brokers', [
    '# DAT Load Posting Guide (Broker Mode)',
    '',
    '## Overview',
    'When OnTrack has freight to move (operating as a broker), you post the load to DAT so carriers and dispatchers can find it and call in with offers. This guide covers how to post effectively to attract qualified carriers quickly.',
    '',
    '## Before You Post',
    'Have all load details confirmed and in hand:',
    '- Exact pickup address and appointment window',
    '- Exact delivery address and appointment window',
    '- Equipment type and trailer length',
    '- Weight and commodity',
    '- Total miles (verify -- do not rely on estimates)',
    '- Your target carrier pay rate',
    '',
    '## Posting on DAT One',
    '',
    '### Step 1 -- Log In',
    'Go to dat.com and sign in to your DAT One account. You need a broker-level or posting-enabled account to post loads.',
    '',
    '### Step 2 -- Create a New Posting',
    'Click Post Loads or New Posting. Select your company profile.',
    '',
    '### Step 3 -- Enter Load Details',
    '',
    '**Origin:**',
    '- City and state of pickup',
    '- Set radius (start at 50 miles -- expand if no response after 2-3 hours)',
    '',
    '**Destination:**',
    '- City and state of delivery',
    '- Set radius (50 miles is standard)',
    '',
    '**Equipment type:**',
    'Select the correct type: Van, Reefer, Flatbed, Step Deck, etc.',
    'Add trailer length if load requires a specific size (e.g. 48ft or 53ft).',
    '',
    '**Pickup date:**',
    'Enter the pickup date. If the pickup window spans two days, use the first available date.',
    '',
    '**Rate:**',
    'You have two options:',
    '- Post a specific dollar amount: carriers see the rate upfront. Faster response.',
    '- Post "call for rate" (no rate shown): requires negotiation on every call. Use when market is strong and you expect multiple offers.',
    '',
    'For speed, posting a rate just below your target encourages callbacks and gives you room to move.',
    '',
    '**Notes field:**',
    'Include any details carriers need before calling:',
    '- "Drop and hook at pickup"',
    '- "Live unload at delivery -- allow 2 hours"',
    '- "Team load -- must have 2 drivers"',
    '- "New authority welcome -- call to discuss"',
    'Do not put the rate in the notes if you want to negotiate.',
    '',
    '### Step 4 -- Set Visibility',
    'Standard posting: visible to all DAT carriers.',
    'If you have preferred carriers in your DAT network, you can alert them directly through DAT.',
    '',
    '### Step 5 -- Submit the Posting',
    'Review all details, then submit. The load will be visible to searching carriers immediately.',
    '',
    '## Managing Your Active Posting',
    '',
    '**No calls after 2-3 hours:**',
    '- Expand the origin radius to 100-150 miles',
    '- Consider increasing the posted rate by $50-100',
    '- Check if the equipment type is correct',
    '',
    '**Too many calls at a bad rate:**',
    '- If carriers are calling but only offering below-market rates, your posted rate may be too low',
    '- Increase the rate on the posting',
    '',
    '**Load is covered:**',
    'Remove the posting from DAT as soon as you book a carrier. Leaving a covered load posted wastes carrier time and damages your reputation on the board.',
    '',
    '## Rate Strategy',
    '',
    '**Tight market (low truck supply):**',
    'Post slightly above DAT rate average. You will get fast response.',
    '',
    '**Loose market (high truck supply):**',
    'Post at or just below DAT average. Carriers are competing for loads.',
    '',
    '**Urgent load (same-day pickup):**',
    'Post 10-20% above market and note "Urgent -- same-day pickup." Speed premium.',
    '',
    '## After Booking',
    'Once you select a carrier:',
    '1. Remove or mark the posting as covered on DAT',
    '2. Issue the rate confirmation to the carrier',
    '3. Update the load in OnTrack to Booked',
    'See [[Broker Load Lifecycle]] for the full workflow after booking.',
  ].join('\n'), null, null, null)

  // ── 136 ── Inbound Carrier Call Script ────────────────────────────────────
  ups.run(136, 'Inbound Carrier Call Script', 'Brokers', [
    '# Inbound Carrier Call Script',
    '',
    '## Overview',
    'When OnTrack posts a load to DAT, carriers and dispatchers call in to offer their trucks. This is the opposite of your dispatch workflow -- you are now the broker receiving calls rather than making them. Handle every call professionally, vet quickly, and book the best-qualified carrier.',
    '',
    '## Before the Call',
    'Keep the load details open in front of you:',
    '- Pickup address and appointment window',
    '- Delivery address and window',
    '- Equipment requirement',
    '- Target carrier pay rate',
    '- Any special requirements (team, drop-and-hook, etc.)',
    '',
    '## Opening',
    '"OnTrack Hauling Solutions, this is Chris. How can I help you?"',
    '',
    'Or if caller IDs show a carrier area code:',
    '"OnTrack Hauling Solutions, Chris speaking. Are you calling about one of our postings?"',
    '',
    '## Step 1 -- Identify the Load They Are Calling About',
    '"Which load are you calling about? Do you have a reference number or city?"',
    '',
    'Note: carriers do not always have the reference number. They may say "your Dallas to Chicago posting."',
    'Pull up the matching load immediately.',
    '',
    '## Step 2 -- Get Basic Carrier Info',
    '"Great -- what MC number are you calling on?"',
    '',
    'Write it down. Verify at safer.fmcsa.dot.gov in a separate tab while continuing the call.',
    '',
    '"And your company name? And the driver\'s name who would be running this?"',
    '',
    '## Step 3 -- Confirm Equipment and Location',
    '"What equipment do you have available? [Confirm type and length match the load.]"',
    '',
    '"Where is the truck located right now? How many miles from [pickup city]?"',
    '',
    'Deadhead under 150 miles is acceptable. Over 200 miles: use judgment based on truck availability.',
    '',
    '## Step 4 -- Confirm Availability',
    '"When can the driver be at the pickup by?"',
    '',
    'If their ETA does not fit the pickup window: "Our shipper needs the load picked up by [time]. Is there any way to make that window?"',
    'Do not stretch the window to fit -- the shipper appointment is fixed.',
    '',
    '## Step 5 -- Discuss Rate',
    'If you posted a rate: "The rate is [$X] all-in. Does that work for you?"',
    '',
    'If they counter: "I appreciate that, but we are firm at [$X] on this load. The lane and timing support that rate."',
    '',
    'If rate is negotiable: "I am showing [$X] -- can you work with that? I have other calls coming in on this load."',
    '',
    '## Step 6 -- Quick Vetting During the Call',
    'While talking, complete these checks (open in a separate tab):',
    '- FMCSA: authority active? Any OOS orders?',
    '- Authority age: under 30 days = high risk, proceed with caution or decline',
    '- DAT carrier profile if available: any rejection flags?',
    '',
    'If something is wrong: "Thanks for calling -- I need to verify a few things on our end. Can I call you back in 10 minutes?"',
    'This gives you time to check without embarrassing the carrier if it turns out fine.',
    '',
    '## Step 7 -- Close or Decline',
    '',
    '### If the carrier qualifies:',
    '"Great -- I would like to book you on this load. I am going to send a rate confirmation to [email address]. Please review and send back a signed copy before the driver heads to pickup. We will also need a COI with our company listed as certificate holder -- can you have your insurance agent email that over today? Carrier name is OnTrack Hauling Solutions."',
    '',
    '### If the carrier does not qualify (bad authority, wrong equipment, too far):',
    '"Thanks for calling. I do not think this load is going to be the right fit on this one, but I appreciate you reaching out. I will keep your information for future loads."',
    '',
    'Do not explain the specific reason if it is an authority or vetting issue -- just decline professionally.',
    '',
    '## After Booking',
    '1. Remove the DAT posting immediately',
    '2. Log the carrier in OnTrack Brokers section with MC number and contact info',
    '3. Note the equipment type, lanes they cover, and whether they were easy to work with',
    '4. Send the rate confirmation and collect the signed copy before the truck moves',
    '5. Update the load status in OnTrack to Booked',
    '',
    '## When Multiple Carriers Call',
    'If several carriers call on the same posting:',
    '- Take basic info from each caller and say "I will confirm within 30 minutes"',
    '- Vet them in parallel',
    '- Book the best-qualified carrier first',
    '- Call the others back promptly to let them know the load is covered',
    'Carriers remember who calls them back and who ghosts them.',
    '',
    '## Related Documents',
    'See [[Broker Load Lifecycle]] for the full load workflow.',
    'See [[DAT Load Posting Guide (Broker Mode)]] for how to create the posting.',
    'See [[How to Vet a New Broker]] for the reverse scenario -- vetting a broker when you are dispatching.',
  ].join('\n'), null, null, null)
