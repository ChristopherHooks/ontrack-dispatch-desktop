...
const ACTIVE_STATUSES = ['Booked', 'Picked Up', 'In Transit', 'Carrier Selected']

// Broker summary subqueries — only evaluated for broker-mode rows (guarded by load_mode check)
const BROKER_SUMMARY =
  ", CASE WHEN l.load_mode='broker' AND EXISTS (SELECT 1 FROM carrier_offers  WHERE load_id=l.id AND status='Accepted') THEN 1 ELSE 0 END AS has_accepted_offer" +
  ", CASE WHEN l.load_mode='broker' AND EXISTS (SELECT 1 FROM broker_carrier_vetting WHERE load_id=l.id) THEN 1 ELSE 0 END AS has_vetting"

export function listLoads(db: Database.Database, status?: string): Load[] {
  if (status) {
    return db.prepare('SELECT l.*' + BROKER_SUMMARY + ' FROM loads l WHERE l.status = ? ORDER BY l.pickup_date ASC').all(status) as Load[]
  }
  return db.prepare('SELECT l.*' + BROKER_SUMMARY + ' FROM loads l ORDER BY l.pickup_date ASC, l.created_at DESC').all() as Load[]
}
...
export function createLoad(db: Database.Database, dto: CreateLoadDto): Load {
  const r = db.prepare(
    ...
    'trailer_type, commodity, status, invoiced, notes, load_mode) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)'
    ...
    dto.status, dto.invoiced ?? 0, dto.notes ?? null, dto.load_mode ?? 'dispatch')
...
export function updateLoad(db: Database.Database, id: number, dto: UpdateLoadDto): Load | undefined {
  ...
  db.prepare(
    ...
    'dest_city=?,dest_state=?,pickup_date=?,delivery_date=?,miles=?,deadhead_miles=?,rate=?,fuel_surcharge=?,dispatch_pct=?,' +
    'trailer_type=?,commodity=?,status=?,invoiced=?,notes=?,load_mode=?,updated_at=? WHERE id=?'
    ...
    m.status, m.invoiced, m.notes, m.load_mode ?? existing.load_mode, now, id)
