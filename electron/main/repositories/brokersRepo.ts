...
export function createBroker(db: Database.Database, dto: CreateBrokerDto): Broker {
  const r = db.prepare(
    'INSERT INTO brokers (name, mc_number, phone, email, payment_terms, credit_rating, avg_days_pay, flag, notes, new_authority, min_authority_days, credit_limit, contact_type) ' +
    'VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'
...
export function updateBroker(db: Database.Database, id: number, dto: UpdateBrokerDto): Broker | undefined {
  ...
  db.prepare('UPDATE brokers SET name=?,mc_number=?,phone=?,email=?,payment_terms=?,credit_rating=?,avg_days_pay=?,flag=?,notes=?,new_authority=?,min_authority_days=?,credit_limit=?,contact_type=?,updated_at=? WHERE id=?')
    .run(m.name, m.mc_number, m.phone, m.email, m.payment_terms, m.credit_rating, m.avg_days_pay, m.flag, m.notes, m.new_authority ?? 0, m.min_authority_days ?? null, m.credit_limit ?? null, m.contact_type ?? existing.contact_type, now, id)
